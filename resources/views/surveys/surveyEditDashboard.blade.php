...
<form method="POST" action="{{ url('editSurvey/')}}" id="surveyHiFPT" >
    {!! csrf_field() !!}

    <input type="hidden" value="{{$accountInfo['ObjID']}}" name="objID">
    <input type="hidden" value="{{$accountInfo['LocationID']}}" name="LocationID">
    <input type="hidden" value="{{$surveyInfo['section_survey_id']}}" name="typeSurvey">
    <input type="hidden" value="{{$surveyInfo['section_contract_num']}}" name="contractNum">
    <input type="hidden" value="{{$surveyInfo['section_code']}}" name="codedm">
    <input type="hidden" value="{{$surveyInfo['section_id']}}" name="sectionID">
    <input type="hidden" value="{{$surveyInfo['section_count_connected']}}" name="sectionCountConnected">
    <input type="hidden" value="{{Session::has('languageLocale') ? Session::get('languageLocale') : 'en'}}" name="lang">
        ...
        <div class="col-xs-12 fix-pad">
            <div class="col-xs-12 fix-pad" >
                <div class="radio-survey">
                    <?php
                    $connected = (old('connected') !== null) ? old('connected') : $surveyInfo['section_connected'];
                    ?>
                    <ul class="ul-radio-rate">
                        ...
                        @foreach($arrayConnected as $connectedID => $connectedKey)
                            <li>
                                <label style="{{($connected == 4 && $connectedID != 4) ? 'cursor: not-allowed;' : ''}}" >
                                    <input type="radio" name="connected" idConnection="{{$connectedID}}" {{($connected == $connectedID) ? 'checked="checked"' : ''}} class="show-box3 survey-question connection"  value="{{$connectedID}}" {{($connected == 4 && $connectedID != 4) ? 'disabled' : ''}}>
                                    <span class="verticle-radio">{{trans('connected'.'.'.$connectedKey)}}</span>
                                </label>
                            </li>
                        @endforeach
                    </ul>
            ...
            <div class="col-xs-12 fix-pad">
                <div name="radio-survey-1" class="radio-survey type-survey" style="margin: 0px;">
                    <?php
                        $type = (old('type') !== null) ? old('type') : $surveyInfo['section_survey_id'];
                    ?>
                    <ul class="ul-radio-rate">
                        ...
                        @foreach($arrayPointOfContact as $pointOfContactID => $pointOfContactKey)
                            <li>
                                <label style="{{($type != $pointOfContactID) ? 'cursor: not-allowed;' : ''}}"> <input type="radio" name="type" class="md-primary group-question after-deploy survey-question" value="{{$pointOfContactID}}" {{($type == $pointOfContactID) ? 'checked="checked"' : 'disabled'}}><span class="verticle-radio">{{trans('pointOfContact'.'.'.$pointOfContactKey)}}</span></label>
                            </li>
                        @endforeach
                    </ul>
                </div>
            ...
            <div class="col-xs-12 padding-10 border-gray">
                <?php
                    $name = (old('contactName') !== null) ? old('contactName') : $surveyInfo['section_contact'];
                    $phone = (old('contactPhone') !== null) ? old('contactPhone') : $surveyInfo['section_contact_phone'];
                    $note = (old('note') !== null) ? old('note') : $surveyInfo['section_note'];
                ?>
            ...
            <div class="col-xs-12 fix-pad border-gray">
                <?php
                    $action = (old('action') !== null) ? old('action') : $surveyInfo['section_action'];
                ?>
                <ul class="ul-radio-rate">
    ...
    <div id="questionBoard" class="col-xs-12 border-gray" style="@if($connected != 4){{"display:none;"}}@endif padding-top: 10px">
        ...
        <div id="question{{$questionID}}" class="panel panel-default panel-default-me">
            <div class="panel-heading">
                <h3 class="panel-title">
                    <span class="fa fa-question-circle"></span> <span class="question-title">{{trans('common.Question')}} {{$i++}}</span>: {{trans('question'.'.'.$base['questionKey'])}}? <b title="{{$base['seeMore']}}" style="cursor: default;">{{trans('more.MoreInfo')}}</b>
                </h3>
            </div>
            <div class="panel-body two-col">
                <div class="radio-survey col-xs-12" style="padding-bottom: 15px;">
                    @foreach($base['answers'] as $answerID => $answer)
                    <div class="@if(!in_array($base['questionAlias'], [10])) col-xs-2 @else col-xs-1 @endif">
                        <label style="cursor: pointer">
                            <input idQuestion="{{$questionID}}" type="checkbox" name="rateScore{{$questionID}}" class="survey-question question" {{($customerAnswerID == $answerID)? 'checked="checked"' : "" }} value="{{$answerID}}"/>
                            <span class="verticle-radio">
                                @if(!in_array($base['questionAlias'], [10]))<img class="emo-resize" style="width: 20px;" src="{{asset("assets/img/Point_0".$answer['answerPoint'].".png")}}">@endif
                                {{trans('answer'.'.'.$answer['answerKey'])}}
                                @if(!in_array($base['questionAlias'], [10]))({{$answer['answerPoint']}})@endif
                            </span>
                        </label>
                    </div>
                    @endforeach
                ...
                <div class="col-xs-12" style="padding-top: 15px;">
                    ...
                    @if(in_array($base['questionAlias'],[5,6]))
                        <div class="col-xs-3">
                            ...
                            <select class="col-xs-10" name="extraAction{{$questionID}}" {{(empty($customerAnswerID) || !in_array($customerAnswerID, [1,2])) ? 'disabled':""}} style="{{(empty($customerAnswerID) || !in_array($customerAnswerID, [1,2])) ? "background-color: gainsboro":""}}">
                                <option disabled selected>--{{trans('common.Select')}}--</option>
                        ...
                        <div class="col-xs-3">
                            ...
                            <select class="col-xs-10" name="extraError{{$questionID}}" {{(empty($customerAnswerID) || !in_array($customerAnswerID, [1,2])) ? 'disabled':""}} style="{{(empty($customerAnswerID) || !in_array($customerAnswerID, [1,2])) ? "background-color: gainsboro":""}}">
                                <option disabled selected>--{{trans('common.Select')}}--</option>
        ...
        @foreach($baseQAOther as $questionID => $base)
            <div id="question{{$questionID}}" class="panel panel-default panel-default-me">
                <div class="panel-heading">
                    <h3 class="panel-title">
                        <span class="fa fa-question-circle"></span> <span class="question-title">{{trans('common.Question')}} {{$i++}}</span>: {{trans('question'.'.'.$base['questionKey'])}}? <b title="{{$base['seeMore']}}" style="cursor: default;">{{trans('more.MoreInfo')}}</b>
                ...
                <div class="panel-body two-col">
                    <div class="radio-survey col-xs-12" style="padding-bottom: 15px;">
                        <?php $countTable = 0; ?>
                        @foreach($base['answersGroup'] as $answerGroupID => $answerGroup)
                            <?php
                                $customerNote = !empty(old('subNote' . $questionID)) ? old('subNote' . $questionID) : ((isset($customerQA[$questionID])) ? $customerQA[$questionID]['survey_result_note'] : null);
                                $row = round(count($answerGroup['answers'])/2);
                            ?>
                            <div class="col-xs-6" style="padding-bottom: 15px;">
                                <table class="col-xs-12" style="border: 1px solid black; border-collapse: collapse;">
                                    <tr>
                                        <?php $countRow = 1; ?>
                                        @foreach($answerGroup['answers'] as $answerID => $answer)
                                            <?php
                                                $customerAnswerID = null;
                                                if(!empty(old('rateScore' . $questionID))){
                                                    if(in_array($answerID, old('rateScore' . $questionID))){
                                                        $customerAnswerID = $answerID;
                                                    }
                                                }else{
                                                    if(isset($customerQA[$questionID])){
                                                        if(is_array($customerQA[$questionID]['survey_result_answer_id'])){
                                                            if(in_array($answerID, $customerQA[$questionID]['survey_result_answer_id'])){
                                                                $customerAnswerID = $answerID;
                                                            }
                                                        }else{
                                                            if($answerID == $customerQA[$questionID]['survey_result_answer_id']){
                                                                $customerAnswerID = $answerID;
                                                            }
                                                        }
                                                    }
                                            ...
                                            ?>
                                            @if($countRow == 1)
                                                <td rowspan="{{$row}}" style="width: 20%; text-align: center;vertical-align: middle; border: 1px solid black;">
                                                    <div>{{trans('answer'.'.'.$answerGroup['answerGroupKey'])}}</div>
                                                </td>
                                            @endif
                                            <td @if($row == 1) style="width: 80%; border: 1px solid black;" @else style="width: 40%; border: 1px solid black;" @endif >
                                                <label style="cursor: pointer; margin-left: 10px;">
                                                    <input idQuestion="{{$questionID}}" type="checkbox" name="rateScore{{$questionID}}[]" class="survey-question" {{($customerAnswerID == $answerID)? 'checked="checked"' : "" }} value="{{$answerID}}"/>
                                                    <span class="verticle-radio">
                                                        {{trans('answer'.'.'.$answer['answerKey'])}}
                                                    </span>
                                                </label>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{trans($transFile.'.Back')}}</button>
                <button type="button" class="btn btn-primary" id="btnSubmit">{{trans($transFile.'.Update')}}</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function () {
          ...
          $('.question').click(function () {
            ...
            if ($box.is(":checked")) {
                if ($box.val() == 1 || $box.val() == 2) {
                    ...
                    disabledActionError(idQuestion)
                }
        ...
        function disabledActionError(idQuestion)
        {
